...
interface ProfileFormProps {
  user: StoredUser
  onUpdate?: (data: ProfileInformationValidation) => Promise<{ success: boolean }>
}

export function ProfileForm({ user, onUpdate }: ProfileFormProps) {
  const [isEditing, setIsEditing] = useState(false)
  const { handleSubmit, reset, watch, setValue, trigger, formState: { errors, isSubmitting } } = useForm<ProfileInformationValidation>({
  ...
  const getBeltDisplay = () => {
    ...
    const stripe = watch("beltStripe") && watch("beltStripe") !== BeltStripe.NONE
      ? ` (${watch("beltStripe")} stripe${watch("beltStripe") !== BeltStripe.ONE ? "s" : ""})`
      : ""
    return `${belt}${stripe}`
  }
        ...
        <div className="space-y-2">
          ...
          {isEditing ? (
            <Select
              ...
              onValueChange={(value) => setValue("beltLevel", value as UserBeltLevel)}
            >
        ...
        <div className="space-y-2">
          ...
          {isEditing ? (
            <Select
              ...
              onValueChange={(value) => setValue("beltStripe", value as BeltStripe)}
            >
                ...
                {Object.values(BeltStripe).map((stripe) => (
                  <SelectItem key={stripe} value={stripe}>
                    {stripe === BeltStripe.NONE 
                      ? "No Stripes" 
                      : `${stripe} Stripe${stripe !== BeltStripe.ONE ? "s" : ""}`}
                  </SelectItem>
                ))}
              </SelectContent>
            </Select>
